...
const Home = (props) => {
    ...
    const getTitleData=async()=>{
        Axios.get('http://192.168.152.254:5000/audio/title/all').then(res => {
            setTitle(res.data.titleData)
        }).catch(err => console.log(err))
    }

    const getCategoryData=async()=>{
        Axios.get('http://192.168.152.254:5000/audio/category/all').then(res => {
                    ...
                    <ScrollView
                        ...
                        contentContainerStyle={{ marginTop: 5 }}>
                        {
                            category.map((data, i) => {
                                ...
                                return (
                                    i == 0 ? <Card title={data.category_name} description={data.category_decscription} source={{ uri: 'http://192.168.152.254:5000/' + data.filename }}  style={{ marginRight: 20, backgroundColor: '#95d3e9', marginBottom:10 }} key={i} click={() => props.navigation.navigate('Subcategory', { category_id: data._id,image: 'http://192.168.152.254:5000/' + data.filename,text:data.category_name })} /> :
                                        <Card title={data.category_name} description={data.category_decscription} source={{ uri: 'http://192.168.152.254:5000/' + data.filename }} style={{ marginRight: 20, backgroundColor: '#95d3e9', marginBottom:10 }} key={i} click={() => props.navigation.navigate('Subcategory', { category_id: data._id,image: 'http://192.168.152.254:5000/' + data.filename,text:data.category_name })} />
                                )
                                }
                            })
